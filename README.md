...
- **迭代优化重复检测**（已实现，待实际验证）：`_is_duplicate_of_neighbors()` 在采纳 LLM 精简/扩展结果前检查与相邻段的字符重叠率（阈值 60%），重复内容不予采纳并打印警告
- **过短片段扩展**（已实现，待实际验证）：`_expand_with_llm()` 调用 LLM 扩展过短翻译，时间对齐阶段对仍过短的片段采用轻微降速（0.85x）+ 静音填充居中放置
- **去重函数兜底**（已实现，待实际验证）：`deduplicate_segments()` 在 Whisper 转录后和翻译后各调用一次，清理完全相同或子串包含的连续重复片段
