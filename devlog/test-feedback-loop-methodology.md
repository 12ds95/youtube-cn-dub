...
## Char Hint 校准反馈循环 (2026-05-03)

### 问题

Pass 1 翻译的字数提示 (char hint) 使用固定 `CHARS_PER_SEC = 4.0`，与下游 jieba Ridge v2 时长估算器不一致。jieba 估算器有 +1210ms 截距，导致：
- 短段 (<3s): hint 偏高 +85%，LLM 翻译过长 → 触发 refine
- 长段 (>10s): hint 偏低 +96%，LLM 翻译过短 → 信息丢失

### 方法：网格搜索 + 测试反馈 3 轮迭代

**数据**: 3 个视频 1589 个真实段，反推理想字数 (ideal_chars = actual_chars / jieba_ratio)。

**搜索**: 遍历 intercept ∈ [0, 1500, 50], rate ∈ [150, 400, 10]，最小化 |hint - ideal_chars| / ideal_chars。

**结果**: 最优参数 `(intercept=400, rate=230)`，公式 `target_chars = max(2, int((target_ms - 400) / 230))`。

| 公式 | <2s | 2-4s | 4-6s | 6-10s | 全局MAE |
|------|-----|------|------|-------|---------|
| 旧 4.0 CPS | +85% | +7% | +4% | +5% | 16% |
| jieba 原始 (1210/172) | -9% | -2% | +14% | +27% | **22%** |
| **网格搜索 (400/230)** | +48% | **+2%** | **+4%** | **+8%** | **15%** |

### 迭代验证 (zjMuIxRvygQ, 73 段)

| 轮次 | 改动 | 信息比 | 窗口比 | jieba ratio | 适配率 |
|------|------|--------|--------|-------------|--------|
| 基线 (旧 hint) | — | 0.57 | 0.74 | 1.14 | 44% |
| v1 (400/230, 75-130%) | 公式+宽范围 | **0.67** | **0.87** | 1.32 | 26% |
| v2 (400/230, 85-115%) | 收紧范围 | 0.67 | 0.86 | 1.30 | 37% |
| **v3 (400/230, 80-120%)** | **最终方案** | **0.64** | **0.83** | **1.25** | **36%** |

v2 遵循率反降 (34%<45%)，范围过紧不可行。v3 在信息完整度和时长控制间取得平衡。

### 关键发现

1. **LLM hint 遵循率天然有限 (~45%)**：IWSLT 2025 研究佐证"LLM 忽略显式字数指令"
2. **信息完整比精确字数更重要**：旧翻译 info=0.57 是多轮 refine 压缩的结果，信息已不可逆丢失
3. **正确策略是"初始翻译偏宽，refine 做轻度压缩"**：v3 仅 19% 段需 refine (ratio>1.5)，比旧方案"过紧 hint → 大幅压缩 → 信息丢失"更优

### 代码变更

4 处统一: `_translate_llm` batch + single, `_isometric_translate_batch`, `_isometric_expand_batch`
- 公式: `CHARS_PER_SEC = 4.0` → `max(2, int((target_ms - 400) / 230))`
- 范围: 75%-130% → 80%-120%
- prompt: `"请尽量...信息完整不遗漏"` → `"严格控制...超出范围将被退回重译"`

## 集成测试验证 (2026-05-03, --integrated)

```
=== 集成测试 迭代 1 ===
时间: 2026-05-03 11:25
测试命令: bash test_pipeline.sh --integrated
视频: zjMuIxRvygQ (73 段)
开启功能: two_pass, refine×3, pre_tts_check, tts_feedback, llm_feedback, post_calibration
总耗时: 174s
...
- 覆盖率: 73/73 = 100%  ✅
- 相邻重复: 0/72 = 0%  ✅
- GT 信息比: 0.60 (窗口 0.79)  ✅ (较旧翻译 0.57 +5%)

语速指标:
- raw_ratio 均值: 1.052  ✅ (目标 0.90~1.10)
- raw_ratio 标准差: 0.129  ✅ (目标 <0.15)
- [0.85-1.15] 合规率: 90.4%  ✅ (目标 >85%)
- 离群>1.4: 1  ✅ (目标 ≤5)

TTS 质量:
- CPS mean=4.34  ✅  p95=5.61  ✅
- 等时合规率: 82.2%  ✅
- Jitter: 0.030  ⚠️ (edge-tts 固有, 非管线问题)

Pipeline 各阶段:
- Pre-TTS 预检: 16 段超标 → 11 成功, 5 拒绝 (69%)  ✅
- TTS 反馈闭环: 10 段偏差>15% → 8 改善 (80%)  ✅
- LLM 闭环: 4 段 → 1 调整, 3 重复跳过  ⚠️
- 后校准: 4 段超1.3x → 0 精简成功 (0%)  ❌

问题发现:
- 后校准 0% 成功率: #38/#39/#40 连续段先被 LLM 闭环标记为重复跳过, 后校准时 LLM 也未能精简
- 段 #69 含长 URL (eater.net/...), TTS 时长 9672ms 远超目标 4640ms, 无法靠精简修复

产出文件:
- ✅ final.mp4 (76.0 MB)
- ✅ chinese_dub.wav, subtitle_bilingual.srt, subtitle_zh.srt, subtitle_en.srt
- ✅ tts_segments/ (73 个)

结论: 翻译质量+语速+TTS 全部达标。后校准需改进 (连续段重复检测过于保守)。
```
