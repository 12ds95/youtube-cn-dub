...
当前版本（新增 NLLB 本地离线层）:
```
LLM 批量翻译 → LLM 逐条翻译（含重试）→ NLLB 本地翻译 → Google Translate → 保留原文
```

> NLLB 层: `_translate_nllb_fallback()` 使用 ctranslate2 + sentencepiece 本地推理，无需网络。
> 详见 `docs/research/2026-05-03-nllb-translation-model.md`
...
当前版本: 已替换为 `_estimate_duration_jieba` — jieba 分词 + Ridge 回归校准 (8 特征 + 截距)，无额外乘数。
> 详见 `devlog/2026-05-01-speed-estimation-fix.md` → 迭代 3-4
