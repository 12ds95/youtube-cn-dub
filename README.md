...
- **翻译长度匹配问题**：中文翻译与英文原文很难一一匹配，导致部分片段配音被加速或降速。当前通过迭代优化（`--refine`）精简过长的翻译来缓解加速问题，但降速场景（翻译过短）尚未处理。后续可考虑：
  - 检测降速片段，调用 LLM 补充细节或调整断句
  - 在时间对齐阶段对过短片段做静音填充或语速微调

- **语句重复问题**：偶尔仍会出现字幕或配音语句重复的现象。可能来源：
  - Whisper 英文转录阶段的重复识别
  - 迭代优化过程中中间步骤的遗留污染
  - 需要进一步排查具体触发条件和修复方案
